...
class CapsMassBalance(models.Model):
    ...
    diff_mass_balance           = models.FloatField()
    conc_per_cap_actual         = models.FloatField()
    release_note                = models.BooleanField()

    def save(self, *args, **kwarg):
        #self.production                  = Productions.objects.get(pk=self.pk)
        self.calculated_mass_powder_mix  = self.production.uniformity.mean*self.production.capsprod.amount_of_caps/1000
        ...
        self.conc_per_cap_actual         = self.production.capsprod.conc_powdermix_per_g_actual*self.production.uniformity.mean/1000
